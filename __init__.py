...
def gconnect():
    ...
    # Assing Email as name if User does not have Google Login
    if "name" in data:
...
def createUser(login_session):
    newUser = User()
    newUser.username = login_session['username']
...
def catalogJSON():
    ...
    # used to deterine the starting and closing brackets for items
    itemcount = 0
...
@login_required
def categoryJSON():
    """ Return JSON of all categories in the catelog """
...
@login_required
def itemJSON():
    """ Return JSON of all items in the catelog """
...
@login_required
def newCategory():
    logging.debug("inside newCategory")
	...
	try:
		...
		logging.debug("exception encountered in adding category")
